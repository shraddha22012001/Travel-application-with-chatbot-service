            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                   Restaurants restaurants  = snapshot.getValue(Restaurants.class);
                    productName.setText(restaurants.getPname());
                    productCity.setText(restaurants.getCity());
                    productDescription.setText(restaurants.getDescription());
                    productAddress.setText(restaurants.getAddress());
                    Picasso.get().load(restaurants.getImage()).into(productImage);
                    String sDestination =restaurants.getAddress();
                    mapBtn.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            if(ActivityCompat.checkSelfPermission(PlaceDetailsActivity.this
                                    , Manifest.permission.ACCESS_FINE_LOCATION)== PackageManager.PERMISSION_GRANTED)
                            {
                                ...
                                ActivityCompat.requestPermissions(PlaceDetailsActivity.this
                                        ,new String[]{Manifest.permission.ACCESS_FINE_LOCATION},44);
                            }

                            Toast.makeText(getApplicationContext(),"Displaying Path To "+ restaurants.getAddress(),Toast.LENGTH_SHORT).show();
